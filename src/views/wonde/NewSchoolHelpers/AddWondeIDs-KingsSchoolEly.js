...
export async function addWondeIDs(selectedSchool) {
  ...
  wondeStudentsTemp.forEach((student) => {
    ...
    // for The Kings School Ely
    if (student.forename === 'Isabelle' && student.surname === 'Steele Thompson') {
      studentKey = compressString(
        student.forename + 'Thompson' + dayjs(student.date_of_birth.date).format('YYYY-MM-DD'),
      )
    }

    let studentKeyOld = compressString(student.forename + student.surname)
    ...
    student.classes.data.forEach((classroom) => {
      ...
      classroom.employees.data.forEach((teacher) => {
        ...
        let teacherKey = compressString(teacher.email)
  ...
  uploadedStudents.forEach((student) => {
    ...
    let studentKeyOld = compressString(student.student.firstName + student.student.lastName)
  ...
  uploadedTeachers.forEach((teacher) => {
    ...
    let teacherKey = compressString(teacherEmail)
